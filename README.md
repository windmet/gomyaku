...
Source Discovery boundary. Its contract and current CAT-00–CAT-26 status are
...
approval, materialization approval, receipt-template, receipt-verification, and
Work State proposal commands are non-destructive; template commands only copy
exact plan coverage and leave execution/result fields blank. The separate apply
command mutates only after explicit review, evidence, backup, and
`--apply-reviewed`.
